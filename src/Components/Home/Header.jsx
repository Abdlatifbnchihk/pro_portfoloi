...
function Header() {
  ...
  return (
    <header className="overflow-x-hidden"> {/* Added overflow-x-hidden to prevent any header-level overflow */}
      <div className={`nav-content fixed top-0 left-0 w-full z-50 overflow-x-hidden backdrop-blur-md transition-all duration-200 flex justify-between items-center ${
        ...
        <div className="w-full max-w-3xl sm:max-w-6xl mx-auto px-3 sm:px-4 lg:px-8 overflow-x-hidden">
          <div className="flex items-center justify-between h-16 overflow-hidden w-full">
            {/* Logo - Made flexible to shrink and truncate on small screens */}
            <div className="min-w-0 overflow-hidden flex-shrink"> {/* Removed shrink-0, added flex-shrink and overflow-hidden */}
              <a href="#" className="block max-w-full truncate">
                <span className="font-semibold text-xs sm:text-base sm:text-lg text-[#92b115ee] truncate"> {/* Smaller text on tiny screens, added truncate */}
                  A.Ben Cheikh
                </span>
              </a>
            </div>
            ...
            {/* Desktop Actions */}
            <div className="desktop-actions hidden md:flex md:items-center md:space-x-4">
              <a
                href="#contact"
            ...
            {/* Mobile Menu Button */}
            <div className="md:hidden flex-shrink-0"> {/* Added flex-shrink-0 to prevent button from shrinking */}
              <button
                onClick={() => setOpen((v) => !v)}
                aria-expanded={open}
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden border-t border-slate-100 bg-white/80 overflow-hidden transform origin-top transition-all duration-300 fixed top-16 left-0 w-full z-30 ${
          open
